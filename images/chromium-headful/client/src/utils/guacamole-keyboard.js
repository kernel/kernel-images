...
Guacamole.Keyboard = function Keyboard(element) {
    ...
    var KeyEvent = function KeyEvent(orig) {
        ...
        /**
         * Whether this keydown was generated by the browser's native key
         * repeat behavior.
         *
         * @type {!boolean}
         */
        this.repeat = !!(orig && orig.repeat);
     ...
     *
     * @param {number} keysym
     *     The keysym of the key to repeat.
     *
     * @return {boolean}
     *     true if event should NOT be canceled, false otherwise.
     */
    this.repeat = function(keysym) {

        // Treat repeat as an initial press if local state was reset
        if (!guac_keyboard.pressed[keysym])
            return guac_keyboard.press(keysym);

        // Modifiers must remain held without generating repeated events
        if (no_repeat[keysym])
            return last_keydown_result[keysym] || false;

        // Remote input expects each repeat as a distinct release/press pair
        ...
        if (first instanceof KeydownEvent) {
            ...
            if (accepted_events.length > 0) {
                ...
                if (keysym) {
                    ...
                    var defaultPrevented = !(first.repeat
                        ? guac_keyboard.repeat(keysym)
                        : guac_keyboard.press(keysym));
