...
type computedState struct {
	...
	// layout_settled: 1 s after page_load with no intervening layout shifts.
...
// stopTimer stops a timer if non-nil and drains its channel.
...
// Must NOT be called with mu held (it acquires mu itself).
...
// onRequest is called on Network.requestWillBeSent.
...
func (s *computedState) onLoadingFinished() {
	...
	s.netTimer = time.AfterFunc(500*time.Millisecond, func() {
		...
		s.netFired = true
		s.navNetIdle = true
		s.publish(events.BrowserEvent{
			Type:        "network_idle",
			Category:    events.CategoryNetwork,
			Source:      events.SourceCDP,
...
// It re-acquires the lock independently.
