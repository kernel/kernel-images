...
func (t *HumanizeMouseTrajectory) tweenPoints(points [][2]float64, opts *Options) [][2]float64 {
	...
	// This produces: small pixel steps at start/end (slow velocity),
	// large pixel steps in the middle (fast velocity) — matching
	// the bell-shaped velocity profile of real human arm movements.
	res := make([][2]float64, targetPoints)
	res[0] = points[0]
