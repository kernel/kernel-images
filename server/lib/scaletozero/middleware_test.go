...
func TestMiddlewareFallsBackToClosedSocketMonitoringAfterAbortFailure(t *testing.T) {
	...
	drain.onComplete = func(value responseDrainOutcome) { outcome <- value }
	abortAttempted := make(chan struct{})
	drain.abort = func(*net.TCPConn) error {
		close(abortAttempted)
	...
	case <-base.enabled:
	case <-time.After(time.Second):
		t.Fatal("scale-to-zero hold was not released after closed-socket monitoring")
	}
...
func TestMiddlewareWriterToResponseRefreshesDeadline(t *testing.T) {
	...
	handler := middleware(NewNoopController(), config)(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		handlerDone <- struct {
			...
			elapsed time.Duration
			err     error
		}{written: written, elapsed: time.Since(started), err: err}
	}))
	...
	result := <-handlerDone
	require.NoError(t, result.err)
	assert.Equal(t, int64(bodySize), result.written)
	assert.Equal(t, int64(bodySize), received)
	assert.Greater(t, result.elapsed, timeout)
	assert.GreaterOrEqual(t, deadlines.Load(), int32(bodySize/responseWriteChunkSize))
}

func TestWriteFailuresProduceOneWriteErrorOutcome(t *testing.T) {
	tests := []struct {
		name         string
		responseLen  int
		wantWriteErr bool
	}{
		{name: "finish_request_flush", responseLen: 32, wantWriteErr: false},
		{name: "handler_write", responseLen: 4 << 10, wantWriteErr: true},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			config := testDrainConfig(time.Second, outboundQueue)
			config.setDeadline = func(*net.TCPConn, time.Time) error { return assert.AnError }
			outcomes := make(chan responseDrainOutcome, 2)
			config.onComplete = func(outcome responseDrainOutcome) { outcomes <- outcome }
			handlerWrite := make(chan error, 1)
			handler := middleware(NewNoopController(), config)(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				_, err := w.Write(make([]byte, tc.responseLen))
				handlerWrite <- err
			}))
			...
			require.NoError(t, err)
			if tc.wantWriteErr {
				require.Error(t, <-handlerWrite)
			} else {
				require.NoError(t, <-handlerWrite)
			}
			select {
			case outcome := <-outcomes:
				assert.Equal(t, responseDrainWriteError, outcome)
			case <-time.After(time.Second):
				t.Fatal("write failure did not complete the response hold")
			}
			select {
			case outcome := <-outcomes:
				t.Fatalf("write failure produced a second outcome: %s", outcome)
...
func testDrainConfig(timeout time.Duration, outbound func(*net.TCPConn) (int, error)) responseDrainConfig {
	...
	config.initialPollInterval = time.Millisecond
